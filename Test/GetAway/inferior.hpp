...
template<typename Master,bool ID, const char*... execArgs>
class inferior {
...
template<typename Master, bool ID, const char *... execArgs>
inferior<Master, ID, execArgs...>::inferior(Master* master_, asio::io_context& io_):
...
template<typename Master, bool ID, const char *... execArgs>
void inferior<Master, ID, execArgs...>::outputReceivedFromInferior(const std::error_code &errorCode) {
    ...
    master->outputReceivedFromInferior(std::move(str));
}

template<typename Master, bool ID, const char *... execArgs>
void inferior<Master, ID, execArgs...>::sendInputToInferior(const std::string& str) {
    write(readingPipeForInferior[1], str.data(), str.size());
}

template<typename Master, bool ID, const char *... execArgs>
void inferior<Master, ID, execArgs...>::receiveOutputFromInferior() {
...
class inferior<Master, true, execArgs...> {
    ...
    int id;
public:
    inferior(Master* master_, int id_, asio::io_context& io_);
    void sendInputToInferior(const std::string& str);
    void receiveOutputFromInferior();
};
...
template<typename Master, const char *... execArgs>
inferior<Master, true, execArgs...>::inferior(Master* master_,int id_, asio::io_context& io_):
        io{io_}, desc{io_}, master{master_}, id{id_}{
    buffer.resize(4096);
    int p = pipe(readingPipeForInferior);
    if(p !=0){
        std::cerr<<"Could Not Open The Pipes In inferior constructor"<<std::endl;
...
template<typename Master, const char *... execArgs>
void inferior<Master, true, execArgs...>::outputReceivedFromInferior(const std::error_code &errorCode) {
    ...
    master->outputReceivedFromInferior(std::move(str), id);
}

template<typename Master, const char *... execArgs>
void inferior<Master, true, execArgs...>::sendInputToInferior(const std::string& str) {
    write(readingPipeForInferior[1], str.data(), str.size());
}

template<typename Master, const char *... execArgs>
void inferior<Master, true, execArgs...>::receiveOutputFromInferior() {
    desc.template async_wait(asio::posix::descriptor_base::wait_type::wait_read,
                             [ptr = this, id = id](const std::error_code& errorCode){
                                 ptr->outputReceivedFromInferior(errorCode);
                             });
}
