...
void clientGetAway::PF::setInputStatementHome3Accumulate(bool clientOnly) {
    clientOnly ? sati::getInstance()->inputStatement = "1)Send Message 2)Leave 3)Exit 4)Perform Turn\r\n" :
            sati::getInstance()->inputStatement = "1)Send Message 2)Close Server 3)Exit 4)Perform Turn\r\n";
    accumulateAndPrint();
}

void clientGetAway::PF::setInputStatementHome3R3(const std::vector<Card>& turnAbleCards_) {
    sati::getInstance()->inputStatement = "Please select one of following:\r\n";
    int count = 1;
    ...
    for(auto& ca: cards){
        if(!ca.second.empty())
            sati::getInstance()->inputStatement += deckSuitValue::displaySuitType[ca.first] + ": ";
