...
io{io_}
{
    nextManager = std::make_shared<serverLobby>(serverlistener, io);
}
...
void serverAuthManager::packetReceivedFromNetwork(std::istream &in, int receivedPacketSize, int excitedSessionId){
    ...
    if(str == password)
    {
        ...
        std::make_shared<session<serverLobby, true>>(std::move(serverAuthSessions.find(excitedSessionId)->second->sock),
